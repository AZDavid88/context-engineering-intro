#!/usr/bin/env python3
"""
Genetic Seed Whitespace Cleanup Utility

This script safely removes excessive empty lines from genetic seed files
while preserving all functional code and proper Python formatting.

Safety Features:
- Preserves all imports, classes, methods, and logic
- Maintains proper indentation and structure
- Reduces excessive consecutive empty lines to maximum 2
...
def main():
    """Clean up the VWAP reversion seed file."""
    seed_file = "src/strategy/genetic_seeds/vwap_reversion_seed.py"
    
    if not os.path.exists(seed_file):
        print(f"File not found: {seed_file}")
        return
    
    print(f"🧹 Cleaning whitespace in {seed_file}")
    ...
    # Clean the file
    success = cleanup_whitespace(seed_file, max_consecutive_empty=1)
    ...
    if success:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("test_seed", seed_file)
